...
public class ExcelUtil2<T> {
    ...
    public boolean exportExcel(List<T> list, String sheetName, int sheetSize,
        ...
        for (int index = 0; index <= sheetNo; index++) {
            ...
            for (int j = 1; j < 3; j++) {
                ...
                if (j == 1) {
                    setMergeColumns(sheet, 1, list.size(), 1, 1);
                    setMergeColumns(sheet, 1, list.size(), fields.size(), fields.size());
                } else {
                    setMergeColumns(sheet, list.size() + 1, 6, 1, 1);
                    setMergeColumns(sheet, list.size() + 1, 6, fields.size(), fields.size());
                }
                ...
                String sumstring = "SUM(C2+C3+C4)";//求和公式
                if (j != 1) {
                    HSSFRow row1 = sheet.getRow(3);
                    System.out.println();
                } else {
                    sheet.getRow(1).getCell(6).setCellValue(456);
                }
    ...
    public boolean exportExcels(List<List<T>> list, String sheetName, int sheetSize,
        ...
        for (int index = 0; index <= sheetNo; index++) {
            ...
            cell = row.createCell(7);// 创建列
            cell.setCellType(CellType.STRING);// 设置列中写入内容为String类型
            cell.setCellValue("总收入");
//            cell = row.createCell(8);// 创建列
//            cell.setCellType(CellType.STRING);// 设置列中写入内容为String类型
//            cell.setCellValue(attr.name());
//            sheet.getRow(0).getCell(7).setCellValue("总收入");
//            sheet.getRow(0).getCell(12).setCellValue("盈利");

            for (int j = 0; j < list.size(); j++) {
                endRow += list.get(j).size();
                if(startRow<endRow){
                    setMergeColumns(sheet, startRow, endRow, 1, 1);
                    setMergeColumns(sheet, startRow, endRow, fields.size(), fields.size());
                }
                int startNo = index * sheetSize;
...
//                sheet.getRow(startRow).getCell(11).setCellFormula(sumstring);
//                sheet.getRow(startRow).getCell(12).setCellFormula(sumstring);
                startRow += endNo;
            }
